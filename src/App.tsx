...
import { Calendar, Activity, ShieldAlert, Award, ChevronLeft, ChevronRight, CheckCircle2, Star, HelpingHand } from 'lucide-react';

export default function App() {
  const [activeTab, setActiveTab] = useState<'workouts' | 'pitchlogs' | 'armcare' | 'rules'>('workouts');
  const [currentDate, setCurrentDate] = useState<string>('2026-06-13');
        ...
        const defaultLogs: PitchLog[] = [
          {
            ...
            topSpeedMph: 41,
            caughtSameDay: false,
            notes: 'Solid warm up performance. Throws were quick, clean follow through.',
          ...
          },
          {
            ...
            topSpeedMph: 44,
            caughtSameDay: false,
            notes: 'Fastball was running and accurate. Took 2 full days of resting.',
          ...
          },
          {
            ...
            topSpeedMph: 46,
            caughtSameDay: false,
            notes: 'Basic throw targets bullpen. Checked the circle change grip draft.',
            clearedDate: '2026-06-12',
            requiredRestDays: 1
          }
        ];
        localStorage.setItem('pitcher_app_pitch_logs', JSON.stringify(defaultLogs));
  ...
  const getOverallReadiness = () => {
    if (pitchLogs.length === 0) return { label: 'Excellent', color: 'bg-[#48BB78] text-white border-[#004E89]' };
  ...
  return (
    <div className="min-h-screen bg-[#F0F4F8] text-[#1A202C] flex flex-col font-sans" id="app-wrapper">
      
      {/* 1. Global Navigation Top Header */}
      <header className="bg-[#004E89] text-white border-b-4 border-[#003B69] shadow-md" id="global-header">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black tracking-tight leading-none uppercase font-display">Youth Slugger Pro</h1>
              <p className="text-xs font-bold text-[#A3BFFA] uppercase tracking-widest mt-1">Arm safety & Summer Coach's Dashboard</p>
            </div>
          </div>

          {/* Interactive Date passage simulator */}
          <div className="flex items-center gap-2 bg-[#003B69] border-2 border-[#004E89] rounded-2xl p-2.5 shadow-inner" id="date-simulator-widget">
            <button 
              onClick={() => shiftDate(-1)}
            ...
            <div className="text-center min-w-[170px] px-1 select-none">
              <p className="text-[9px] uppercase font-bold text-[#A3BFFA] tracking-wider">Simulated Ticker Date</p>
              <p className="text-xs font-black text-[#FFBC42] mt-0.5">{formatReadableDate(currentDate)}</p>
            </div>
      ...
      <div className="bg-white border-b-4 border-[#004E89] py-3 shadow-xs" id="sub-header-bar">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-1 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs font-bold">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[#004E89] font-black uppercase tracking-wider text-xs">Arm Status:</span>
            <span className={`px-4 py-1 text-[11px] font-black rounded-full border-2 border-[#004E89] uppercase ${readiness.color}`}>
              {readiness.label}
            </span>
            {readiness.desc && (
              <span className="text-slate-600 font-bold hidden md:inline">| {readiness.desc}</span>
            )}
          </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 md:px-8 py-6" id="main-content">
        
        {/* Navigation Tabs bar */}
        <div className="flex justify-center md:justify-start mb-8 overflow-x-auto scrollbar-none" id="dashboard-navbar" role="tablist">
        ...
        <div className="focus:outline-none transition-all" tabIndex={0} id="tab-content-renderer">
          {activeTab === 'workouts' && (
      ...
      <footer className="bg-[#003B69] border-t-4 border-[#004E89] text-white text-xs py-6 mt-12" id="global-footer">
